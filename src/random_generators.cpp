...
IntegerVector Sample_int(const unsigned int n,const unsigned int size = n,const bool replace = false){
	IntegerVector res(n);
	if(replace){
...
NumericVector Sample(NumericVector x,const unsigned int size,const bool replace = false){
	...
	if(replace){
		...
		uniform<integer,false> rng(0,x.size()-1);
		for(int i=0;i<size;++i){
			res[i]=x[rng()];
		}
	}
...
RcppExport SEXP Rfast2_Sample_int(SEXP nSEXP,SEXP minSEXP,SEXP maxSEXP){
BEGIN_RCPP
    RObject __result;
    RNGScope __rngScope;

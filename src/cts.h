...
static void combn(T2& vals, const int32_t n, const uint32_t start_idx, 
		std::vector<double>& combn_data, T1& combn_ds, uint32_t& combn_col);
...
static void combn(T2& vals, const int32_t n, const uint32_t start_idx, 
		std::vector<double>& combn_data, T1& combn_ds, uint32_t& combn_col) {
	if (!n) {
		for (uint32_t i = 0; i < combn_ds.n_rows && combn_col < combn_ds.n_cols; i++) {
...
arma::mat calc_pt(arma::mat& ds, 
		const int32_t df, const bool lower_tail, const bool log_p, 
		const double add);

std::vector<uint32_t> det_cols(arma::umat& ds, const uint32_t val);

arma::mat ext_cols(arma::mat& ds,  const uint32_t col_a, 
		const uint32_t col_b);
...
std::unordered_set<uint32_t> get_diff(std::vector<uint32_t>& lh, const uint32_t val);

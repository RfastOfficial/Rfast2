...
//[[Rcpp::export]]
List negbin_regs(NumericVector Y, NumericMatrix X, const double tol = 1e-07, const int maxiters = 100){
  ...
  #pragma omp parallel
  {
    ...
    for(int j=0;j<p;j++){
      ...
      // todo glbpoisson2 should return a custom struct or smth. Not an rcpp class
      vec* mod = new vec[2];
...
RcppExport SEXP Rfast2_negbin_regs(SEXP ySEXP, SEXP xSEXP, SEXP tolSEXP, SEXP maxitersSEXP) {
  BEGIN_RCPP
  RObject __result;
  RNGScope __rngScope;
  ...
  __result = negbin_regs(y,x,tol,maxiters);
  return __result;
  END_RCPP
}
